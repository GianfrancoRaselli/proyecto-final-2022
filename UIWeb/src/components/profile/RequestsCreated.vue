  ...
  <div class="container">
    ...
    <div class="items" v-else>
      <div class="no-items" v-if="requestsToShow && requestsToShow.length === 0">
        ...
        <span>La entidad no ha creado ninguna solicitud aún.</span>
      </div>
      <div v-else>
        <div class="item" :class="getRequestClass(request)" v-for="(request, index) in requestsToShow" :key="index">
          ...
          <div class="content">
            <div class="info">
              ...
              <span class="hover" v-text="funds[request.fundIndex].name" :goToFund="true"></span>
            </div>
  ...
  computed: {
    requestsToShow() {
      ...
      requestsToShow = requestsToShow.filter((request) => {
        return compareAddresses(request.petitioner, this.$route.params.address);
      });
